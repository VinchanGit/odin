...
$toolMessages = [
    ...
    new UserMessage('请查询深圳的天气，并告诉我适合穿什么衣服。'),
];

$start = microtime(true);

// 第一轮：使用流式 API 进行工具调用
echo '>>> 第一轮对话：流式请求天气信息' . PHP_EOL;
echo '流式响应内容: ';
...
// 流式完成后获取完整消息
// 流式完成后，我们可以从 choices 中获取完整消息
$completeMessage = null;
$allChoices = $response->getChoices();
if (! empty($allChoices)) {
    // 获取最后一个 choice，它应该包含完整的消息
...
if ($completeMessage instanceof AssistantMessage) {
    ...
    if (! empty($toolCalls)) {
        ...
        // 第二轮：继续对话，包含所有工具响应（同样使用流式）
        echo PHP_EOL . '>>> 第二轮对话：根据工具结果生成回复（流式）' . PHP_EOL;
        echo '助手最终回复: ';
        ...
        echo '未检测到工具调用' . PHP_EOL;
        if (! empty($streamedContent)) {
            echo '直接响应内容: ' . $streamedContent . PHP_EOL;
        }
        echo PHP_EOL . '这可能意味着：' . PHP_EOL;
        echo '  1. 模型直接回答了问题，没有使用工具' . PHP_EOL;
        echo '  2. 或者工具定义不够明确' . PHP_EOL;
    }
} else {
    echo '响应不是 AssistantMessage 类型' . PHP_EOL;
}
